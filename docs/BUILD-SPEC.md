...
| Land safety checklist (`/guides/land-safety-checklist/`, a guide page, not a tool) | The 30 checks in 5 stages as HTML (headings and lists, ItemList schema), red flags and documents, plus a free PDF download with a page-1 preview and no email gate | none; the PDF is the printable copy | `lib/checklist.ts` (one source for the HTML and the schema; the English matches the PDF), `public/downloads/` |
